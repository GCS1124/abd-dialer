...
import { getBreakMenuOptions } from "../../lib/timeTracking.ts";
...
export function BreakMenu({
  ...
  return (
    <div className="absolute right-0 top-[calc(100%+0.85rem)] z-50 w-[18.5rem]">
      <div className="rounded-[20px] border border-slate-200 bg-white p-3.5 shadow-[0_24px_60px_rgba(15,23,42,0.14)] dark:border-slate-800 dark:bg-slate-950">
        ...
        <div className="mt-3 h-[3px] rounded-full bg-[#2b97ff]" />
        ...
        <div className={cn("mt-3 max-h-[14rem] space-y-2 overflow-y-auto pr-1", onBreak && "mt-3")}>
          {options.map((option) => (
            <button
              key={option.value}
              type="button"
              ...
              className={cn(
                ...
                "border-slate-200 bg-white hover:border-sky-200 hover:bg-sky-50/70 disabled:cursor-not-allowed disabled:opacity-60",
