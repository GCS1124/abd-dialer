...
begin
  workspace_id_to_delete := private.current_workspace_id();
  ...
  if not exists (
    ...
    where au.auth_user_id = auth.uid()
      and au.workspace_id = workspace_id_to_delete
...
revoke all on function public.delete_campaign_queue(uuid, text) from public, anon;
grant execute on function public.delete_campaign_queue(uuid, text) to authenticated;
